...
class Forest:
    ...
    def node_features(self, subset=None):
        ...
        if len(keys) != len(set(keys)):
            ...
            keys_unique = range(len(keys))
        else:
            keys_unique = keys
        result = pd.concat(features, keys=keys_unique, names=names, ignore_index=False)
        return result
    ...
    def mean_site_frequency_spectrum(self, which="binned_normalized_cut"):
        ...
        S = np.empty((len(self.trees), getattr(self._site_frequency_spectrum[0], which).shape[0]))  # array of binned SFS (dimensions are (number of trees, number of bins))
    ...
    def fit_metric(self, attr, model):
        ...
        if not hasattr(self, attr) or getattr(self, attr) == None:
            print "Error in Forest.fit_metric(): Forest has no attribute", attr
            return None
    ...
    def p_metric(self, attr, model, value):
        ...
        p = model.cdf(value, *fit)  # calculate pvalue of value under model with fitted parameters
        return p
