...
class SizeMatchedModel:
    ...
    def __init__(self, bins, params, distribution, name=None):
        ...
        assert len(params) == len(bins) - 1, "Length of params must be one less than length of bins"
        self.bins = bins
        self.params = params
    ...
    def from_json(cls, filename):
        ...
        distribution = eval(attributes_str["distribution"])()  # evaluate class name and instantiate
        return SizeMatchedModel(attributes["bins"], attributes["params"], distribution, attributes["name"])
    ...
    def to_json(self, outfile):
        ...
        distribution_str = self.distribution.__class__.__module__ + "." + self.distribution.__class__.__name__
    ...
    def _params_for_size(self, size, strict_bounds=True):
        ...
        bin_index = np.digitize(size, self.bins)  # digitize returns the index of the bin to which value belongs
        ...
        if (bin_index == 0 or bin_index == len(self.bins)) and strict_bounds:
            ...
            raise ValueError("Size must be within bounds of bins (if strict_bounds=True)")
