...
def calculate_tree(aln, outgroup, ancestral=True):
    ...
    ps = subprocess.Popen(('gunzip', '-c', tmp_file_name), stdout=subprocess.PIPE)
    ...
    anc_rec = ancestral.ancestral_sequences(biopython_tree, tmp_aln, copy_tree=False)
...
def annotate_leaf(leaf, annotation):
    ...
    '''
    for attr, val in annotation.iteritems():
...
def translate_sequences_on_tree(T, cds = {'begin':0, 'pad':0, 'end':0}):
    ...
    if verbose:
        tmp_time = time.time()
        print "translating_sequences_on_tree: ...",
    for node in T.get_nonterminals()+T.get_terminals():
        ...
        try:           
            try:
                ...
                from Bio import Seq
                node.aa_seq = Seq.Seq('X'*(cds['pad']+(last_base - cds['begin'])/3))
                print 'Error translating:', node.name, 'replaced sequences by XXX...XXX'
...
def mutations_on_branches(T, aa=False):
    ...
    if verbose:
        tmp_time = time.time()
        print "mutations_on_branches: find all mutations ...",
    T.root.mutations=[]
    if aa:
        T.root.aa_mutations=[]
...
def mutations_on_branches_subtree(subtree, aa = False):
    ...
    for child in subtree.clades:
        try:
            ...
            mutation_pos = np.where(np.array(subtree.seq)!=np.array(child.seq))[0]
            child.mutations = [(pos, subtree.seq[pos], child.seq[pos]) 
                               for pos in mutation_pos]
            # repeat for amino acids
            if aa:
                aa_mutation_pos = np.where(np.array(subtree.aa_seq)!=np.array(child.aa_seq))[0]
                child.aa_mutations = [(pos, subtree.aa_seq[pos], child.aa_seq[pos]) 
...
def find_internal_nodes(sourceT, destT):
    ...
    path_in_destT = {leaf.name:[destT.root]+destT.root.get_path(destT_leaf_lookup[leaf.name]) 
    ...
    for intnode in sourceT.get_nonterminals():
        tmp_path = [path_in_destT[leaf.name] for leaf in intnode.get_terminals()]
        min_length = min([len(a) for a in tmp_path])
        pos=0
        while pos<min_length and all([tmp_path[0][pos]==p[pos] for p in tmp_path[1:]]):
            pos+=1
...
def plot_prediction_tree(prediction, method='mean_fitness', internal = False, fig = None, axes = None, cb = True, scalebar = True, offset=0.0001, node_label_func=lambda x: None):
    '''
    ...
    prediction.color_tree(method=method, offset = offset, n_labels = 1, cmap = my_favorite_cmap, nodes =nodes)
    if not internal:
        prediction.interpolate_color()
    ax = draw_tree(prediction.T, fig = fig, axes = axes, node_label=node_label_func, branch_label=node_label_func, cb=cb, scalebar=scalebar)

def draw_tree(tree, node_label = node_label_func, branch_label = node_label_func,
                  cmap = cm.jet, fig = None, axes = None, cb = True, scalebar = True):
    '''
    ...
    Phylo.draw(tree, label_func = node_label, 
               show_confidence = False,branch_labels = lambda x: None, axes=axes, do_show=False)
#               show_confidence = False,branch_labels = branch_label, axes=axes, do_show=False)
    ...
    if cb:
        ...
        sm._A = []
        cbar = fig.colorbar(sm, ticks=[0,100], shrink=0.5, aspect=10,pad=-0.05)
...
def plot_combined_tree(prediction, combined_data,  method='mean_fitness', internal=False, axes = None, cb=True, offset = 0.0001):
    '''
    ...
    prediction.color_other_tree(nodes, method, offset = offset, n_labels = 1, cmap=my_favorite_cmap)
    if not internal:
