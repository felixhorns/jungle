...
class node_ranking(fitness_inference):
    ...
    def compute_rankings(self):
        ...
        if 'mean_fitness' in self.methods:
            if verbose:
                tmp_t = time.time()
                print "node_ranking: inferring ancestral fitness...",
        ...
        if verbose:
            tmp_t = time.time()
            print "node_ranking: calculating alternative rankings...",
            
        if 'depth' in self.methods or (('expansion_score' in self.methods) and (self.time_bins is not None)):
    ...
    def expansion_score(self):
        ...
        if not self.time_bins:
            print "expansion_score(): need time bins to calculate expansion score"
            return
        
        self.leafs_by_bin = self.sort_leafs_in_time_bins()
        total_counts_by_bin = np.array([len(leaflist) for leaflist in 
                                        self.leafs_by_bin], dtype = float)
        for node in self.non_terminals:
            node.counts_by_bin = np.array([len(leaflist) for leaflist in 
                                           self.sort_leafs_in_time_bins(node)])
            node.temporal_frequency = ((node.counts_by_bin + self.pseudo_count)
                                       /(total_counts_by_bin+self.pseudo_count))
            if len(self.time_bins)>1:
                node.expansion_score = stats.linregress(np.arange(len(self.time_bins)), 
                                                    np.log(node.temporal_frequency))[0]
    ...
    def ranking_quality(self):
        ...
        cut_off = self.terminals[0].mean_fitness - np.sqrt(self.terminals[0].var_fitness)
        ci=1
        while self.terminals[ci].mean_fitness>cut_off:
            ci+=1
        return ci

    def color_tree(self,nodes = None, method = 'mean_fitness',  offset=0.000, 
                   n_labels = 10, scramble=True, cmap = None):
        '''
        ...
        for ci, c in enumerate(nodes):
            ...
            c.color = [int(x*240) for x in 
                       np.array(cmap(int(255.0 * (n_nodes - ci)**1 / n_nodes**1))[:3])]
            if c.is_terminal(): c.branch_length+=offset
            if ci<n_labels:
    ...
    def interpolate_color(self, tree=None):
        ...
        for c in tree.get_nonterminals():
            ...
            for leaf in terms:
                if leaf.color:
                    tmp_color+=np.array([leaf.color.red, leaf.color.green, leaf.color.blue])
                    leaf_count+=1
            if leaf_count:
    ...
    def color_other_tree(self,  nodes, method = 'mean_fitness',offset=0.000, 
                         n_labels = 10, scramble=True, cmap = None):
        '''
        ...
        for c in nodes:
            if c.name in look_up:
                ...
                c.color = (100,100,100)
                c.rank_label = None
        self.color_tree(nodes_to_color, method, offset, n_labels, scramble= scramble, cmap =  cmap)
    ...
    def best_node(self, method = 'mean_fitness', nodes = None):
        ...
        if nodes[0].name is None:
            nodes[0].name = 'best by method '+method+' among #of nodes '+str(len(nodes))
        return nodes[0]
